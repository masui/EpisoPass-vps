...
Various authentication systems have been proposed for replacing
passwords, but none of them are found to be better than
password-related systems\ref{Bonneau:ReplacePasswords}.

Although cognitive password systems have been studied for a
long time\ref{xxx}\ref{ZZZZ},
they were meant to be implemented as a replacement to password-based systems,
and few system has tried to convert episodic memories into passwords.
...
The idea of using episodic memories for user authtication is also not new.
In fact, using episodic memories for selecting password is gnerally recommended[],
and many current computer users are probably using password that are
related their episodic memories in some way.
